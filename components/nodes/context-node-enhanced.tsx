...
export function ContextNodeEnhanced({
  ...
  useEffect(() => {
    if (selected || data.isSelected) {
      setDataFlowAnimation(true);
      const timer = setTimeout(() => setDataFlowAnimation(false), 1500);
  ...
  const textColor = data.textColor || "#7c3aed";
  const dotColor = data.dotColor || "#8b5cf6";
  const borderRadius = data.borderRadius || 24;
  ...
  const getBackgroundStyle = () => {
    ...
    switch (nodeStyle) {
      case "gradient":
        return {
          ...baseStyle,
          background: `linear-gradient(135deg, ${currentColor}, ${dotColor}25)`,
        };
      case "glass":
        return {
          ...baseStyle,
  ...
  const model = data.model || "Context";
  const tags = (data.metaTags || []).slice(0, 3);
  const contextSize = data.contextSize || 0;
    ...
    <div className="relative group">
      ...
      <div
        className={`absolute inset-0 rounded-xl pointer-events-none transition-all duration-700 ${
          selected || data.isSelected || dataFlowAnimation
      ...
      <Card
        className={`${getStyleClasses()} ${
          selected || data.isSelected
            ? "scale-[1.03] ring-2 ring-opacity-20"
            : "hover:scale-[1.02]"
        ...
        <div className={`relative ${config.padding}`}>
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div
              ...
              style={{
                width: `${config.iconSize * 2.8}px`,
                height: `${config.iconSize * 2.8}px`,
      ...
      <style jsx>{`
        @keyframes dataFlow {
          ...
          100% {
            opacity: 0;
            transform: translate(150px, -80px) scale(0);
          }
        }
      `}</style>
    </div>
