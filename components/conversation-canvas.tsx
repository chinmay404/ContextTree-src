...
const initialNodes: Node[] = [
  {
    ...
    data: {
      label: "Welcome to ContextTree",
      content: "Start building your conversation canvas here. Add nodes, connect ideas, and let AI help you explore.",
      isMain: true,
    },
  },
  {
...
export function ConversationCanvas({ className }: ConversationCanvasProps) {
  ...
  useEffect(() => {
    if (status === "authenticated" && isInitialized && dbInitialized && savePreferences.enableAutoSave) {
  ...
  const handleEnhancedAutoSave = async () => {
    if (status === "authenticated" && isInitialized && dbInitialized && currentSessionId) {
      try {
        setIsSaving(true)
        ...
        if (activeConv) {
          ...
          const result = await autoSaveConversation(activeConv, currentSessionId, "timer")
        ...
        if (retryCount < 3) {
          ...
          setTimeout(() => handleEnhancedAutoSave(), Math.pow(2, retryCount) * 1000) // Exponential backoff
        } else {
          toast({
  ...
  }, [savePreferences.saveOnExit, hasUnsavedChanges, currentSessionId, conversations, activeConversation])
    ...
    const loadBackupData = async () => {
      if (activeConversation && status === "authenticated") {
        try {
          ...
          const analytics = await getConversationSaveAnalytics(activeConversation)
      ...
      if (params.source === activeNode || params.target === activeNode) {
        ...
        const newMessage = {
          ...
          content:
            direction === "outgoing"
              ? `Connected to ${
                  nodeType === "mainNode" ? "Main Node" : nodeType === "branchNode" ? "Branch Node" : "Image Node"
                } "${nodeLabel}".`
              : `Connected from ${
                  nodeType === "mainNode" ? "Main Node" : nodeType === "branchNode" ? "Branch Node" : "Image Node"
                } "${nodeLabel}".`,
  ...
  const onSendMessage = useCallback(
    async (content: string) => {
      ...
      try {
        ...
        setMessages((prevMessages: Message[]) => [...prevMessages, newAiMessage])
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="flex flex-col items-center">
          ...
          {forceProceed && <div className="mt-2 text-green-600">Proceeding to canvas without loading data...</div>}
        </div>
      </div>
    )
  }
