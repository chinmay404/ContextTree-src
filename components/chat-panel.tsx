...
// Add this helper function to extract thinking content from messages
const extractThinkingContent = (content: string): { thinking: string | null; message: string } => {
  const thinkRegex = /<think>([\s\S]*?)<\/think>/i
  const match = content.match(thinkRegex)

  if (match && match[1]) {
    // Remove the thinking part from the original message
    const cleanedMessage = content.replace(thinkRegex, "").trim()
    return { thinking: match[1].trim(), message: cleanedMessage }
  }
          ...
          <motion.div
            className={`flex-1 overflow-auto ${
              isExpanded
                ...
                : "p-4"
            } space-y-6 custom-scrollbar ${isExpanded ? "max-h-[calc(100vh-140px)]" : ""}`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            style={{
              ...
              backgroundImage: isExpanded
                ...
                : "none",
              backgroundSize: isExpanded ? "25px 25px" : "auto",
              backgroundOpacity: isExpanded ? 0.03 : 1,
            }}
          >
            {messages.length === 0 ? (
              <motion.div
              ...
              messages.map((message, index) => {
                const { thinking: thinkingContent, message: messageContent } = extractThinkingContent(message.content)
                      ...
                      <div className="flex items-end gap-2 relative">
                        ...
                        <div
                          className={`${
                            isExpanded
                              ? message.sender === "user"
                                ? "bg-primary text-primary-foreground max-w-[85%] md:max-w-[75%] rounded-2xl p-4 shadow-md hover:shadow-lg transition-all duration-200"
                                : "bg-card/95 border border-border/70 shadow-md max-w-[85%] md:max-w-[75%] rounded-2xl p-4 hover:shadow-lg transition-all duration-200"
                              : message.sender === "user"
                                ? "bg-primary text-primary-foreground max-w-[85%] rounded-2xl p-3.5 shadow-sm hover:shadow transition-all duration-200"
                                : "bg-card/95 border border-border/70 shadow-sm max-w-[85%] rounded-2xl p-3.5 hover:shadow transition-all duration-200"
                          } relative ${message.sender === "user" ? "chat-bubble-user" : "chat-bubble-ai"}`}
                          style={{
                            boxShadow:
                              message.sender === "user"
                        ...
                        >
                          {(() => {
                            // For user messages, simply display the content
                            if (message.sender === "user") {
                              return (
                                <p
                                  className={`${isExpanded ? "text-base leading-relaxed" : "text-sm leading-relaxed"}`}
                                >
                                  {message.content}
                                </p>
                              )
                            ...
                            // For AI messages, extract thinking content if present
                            const { thinking, message: cleanedMessage } = extractThinkingContent(message.content)

                            return (
                              <>
                                {thinking && (
                                  <Collapsible className="mb-3 w-full">
                                    <div className="flex items-center gap-2 mb-1.5">
                                      <CollapsibleTrigger asChild>
                                        <Button
                                          variant="outline"
                                          size="sm"
                                          className="h-7 gap-1 text-xs bg-muted/30 border-dashed border-muted-foreground/30 hover:bg-muted/50"
                                        >
                                    ...
                                    <CollapsibleContent className="overflow-hidden">
                                      <div className="p-3 border border-dashed border-muted-foreground/20 rounded-lg bg-muted/20 text-muted-foreground text-xs leading-relaxed whitespace-pre-wrap font-mono">
                                        {thinking}
                                      </div>
                                    </CollapsibleContent>
                                  </Collapsible>
                          ...
                          {onCreateBranchNode && message.sender === "user" && (
                            <div
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                onCreateBranchNode(messageContent)
                                toast({
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-2">
              <div className="flex gap-2">
                <Input
                  ...
                  className={`flex-1 ${
                    isExpanded
                      ? "bg-background/80 border-muted-foreground/20 focus-visible:ring-primary/30 h-12 text-base shadow-md rounded-xl"
                ...
                <Button
                  ...
                  className={`${
                    isExpanded
                      ? "bg-primary text-primary-foreground hover:bg-primary/90 px-6 shadow-md hover:shadow-lg transition-all duration-200 rounded-xl h-12"
