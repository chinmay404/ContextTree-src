...
async function testConnection() {
  ...
  try {
    ...
    for (const table of canvasTables) {
      try {
        ...
        if (result.rows.length > 0) {
          ...
          console.log(
            `ℹ️  Canvas table '${table}' does not exist (will be created by app)`
          );
        }
      } catch (error) {
        console.log(
          `❌ Error checking canvas table '${table}':`,
          error.message
        );
      }
    }
    ...
    if (error.code === "ECONNREFUSED") {
      ...
      console.log(
        "💡 Suggestion: Check the database host/URL in your environment variables"
      );
      ...
      console.log(
        "💡 Suggestion: Database connection is timing out - check network connectivity"
      );
    }
  } finally {
    if (client) {
