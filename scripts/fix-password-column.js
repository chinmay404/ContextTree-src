...
async function main() {
  ...
  try {
    ...
    if (usersStructure.rows.length === 0) {
      log(
        "SUCCESS",
        "No password column found in users table - no fix needed!"
      );
      return;
    }
    ...
    log(
      "INFO",
      `Found password column: ${passwordColumn.data_type}, nullable: ${passwordColumn.is_nullable}`
    );
    ...
    if (passwordUserCount > 0) {
      ...
      log(
        "WARNING",
        "These passwords will remain but the column will be made nullable"
      );
    ...
    if (APPLY) {
      ...
      log(
        "WARNING",
        "Dry run complete. Use --apply to actually make these changes."
      );
    }
  } catch (error) {
