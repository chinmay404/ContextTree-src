...
{{conversation_history}}`,
      ...
      },
      {
        ...
        name: "RAG Enhanced",
        description:
          "Retrieval-Augmented Generation with context prioritization",
        template: `{{system_instructions}}
...
{{conversation_history}}`,
  ...
  assemblePrompt(
    ...
    const fullPrompt = assembledTemplate
      .replace(/\n\s*\n\s*\n/g, "\n\n")
      .trim();
    ...
    const systemPrompt =
      variables.system_instructions || "You are a helpful assistant.";
    const userPrompt =
    ...
    const contextSources = [
      ...assembledContext.systemInstructions.map(
      ...
      ...assembledContext.conversationMemory.map(
        (content: string, index: number) => ({
      ...
      ...assembledContext.customContext.map(
        (content: string, index: number) => ({
  ...
  validatePromptLength(
    ...
    if (prompt.tokenCount > modelLimit) {
      warnings.push(
        `Prompt exceeds model limit (${prompt.tokenCount} > ${modelLimit} tokens)`
      );
      suggestions.push(
        "Consider reducing context or splitting into multiple calls"
      );
    }

    if (prompt.tokenCount > modelLimit * 0.8) {
      warnings.push(
        `Prompt is near model limit (${prompt.tokenCount}/${modelLimit} tokens)`
      );
      suggestions.push(
        "Consider optimizing context to leave room for response"
      );
    }
